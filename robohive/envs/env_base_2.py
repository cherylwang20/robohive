...
class MujocoEnv(gym.Env, gym.utils.EzPickle, ObsVecDict):
    ...
    def _setup(self,
        ...
        vector_len = len(observation['vector'])
        assert not done, "Check initialization. Simulation starts in a done state."
        self.observation_space = gym.spaces.Dict({
            'image': gym.spaces.Box(low=0, high=255, shape=(120, 212, 3), dtype=np.float32),  # Use np.float32 here
            'vector': gym.spaces.Box(obs_range[0]*np.ones(vector_len), obs_range[1]*np.ones(vector_len), dtype=np.float32)  # Ensure consistency in dtype usage
        })
